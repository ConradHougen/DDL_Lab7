...
int adc_freq = 1;
double curr_volt = 0;
char curr_volt_str[10]; // 10 character buffer
...
{  
  ...
  if(LED_ENABLE)
  {
	  if ( (timer32_0_counter%(100/freq)) < ((100/freq)*(1-duty)) )
	  {
  ...
  if(ADC_ENABLE)
  {
	  ...
	  if(!(timer32_0_counter%(100/adc_freq)))
	  {
		  ADCRead( 0 );
		  //while ( !ADCIntDone );
		  //ADCIntDone = 0;
		  ...
		  sprintf(curr_volt_str, " %f ", curr_volt);

		  LPC_UART->IER = IER_THRE | IER_RLS;
		  UARTSend((uint8_t *) curr_volt_str, 10);
		  UARTCount = 0;
